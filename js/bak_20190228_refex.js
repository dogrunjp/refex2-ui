        ...
        function show_heatmap(d){
            get_similar_gene(d).then(function(r){
                ...
                summary_chart.append('div').attr('class', 'similar').html(`
                    Similarity Ranking: <span class="msg" data-target="${c.gid}"><a onclick="get_similar_info(${tid}, '${n}','${sym}');" href="#">${n}</a></span>
                    <button class="btn btn-primary btn-xs showmore" data-toggle="collapse" data-target="#hm${c.gid}">Similarity matrix ...</button>`);
                    // click eventを動的に追加する
