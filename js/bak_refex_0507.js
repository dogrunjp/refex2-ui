...
(function () {
    ...
    var bcnf = {width: (container_width - 240), height: 120, hide: 0 ,margin:30, t:8, b: 30, l:30, r: 30},
        ccnf = {h: 40, t:8, b: 30},
        xb = d3.scaleBand().range([0, bcnf.width - bcnf.l ]), // x scale for focused chart
    ...
    var xAxisb = d3.axisBottom(xb).tickSize(0).tickFormat(''),
    ...
    function summary_chart(c, data){
        ...
        svg.append("g").selectAll('.bar').data(data)
            ...
            .attr("x", function(d){return xs(d.sample)})
    ...
    * 詳細チャート
        ...
        chart: function (data) {
            ...
            focus.selectAll('.f_bar')
                ...
                .on("mouseover", function (d) {
                    show_tips(d.refex, d.value)
                })
                .on("click", function (d, i) {
                    show_reference(d, i, input_data)
                });
    ...
    function brushed() {
        ...
        var x = focus.sorted ? xs: xb;

        selected = x.domain()
            .filter(function (d, i) {
                return s[0] <= (a = (i + 1) * x.bandwidth()) && (a + x.bandwidth() <= s[1])
            });
        xc.domain(selected);

        focus.select(".x-axis").call(xAxisb);
        focus.selectAll(".f_bar")
        .attr('x', function(d){return xc(d.sample)})
        .attr('width', xc.bandwidth())
        .attr('y', function(d){return yb(d.value)})
        .attr('height', function(d){return bcnf.height - yb(d.value)})
        .on("mouseover", function(d){show_tips(d.sample, d.value)})
    ...
    function sort_bars(){
        ...
        var samples_sorted = data_sorted.map(function(d){return d.sample});

        console.log(samples_sorted.length);
        u_data = Array.from(new Set(samples_sorted));
        console.log(u_data.length);
        ...
        focus.selectAll('.f_bar').transition().duration(500)
            .attr('x', function(d){if(d.sample == 'RES00000547'){console.log(xs(d.sample))}; return xs(d.sample);})
            .attr('width', xb.bandwidth())
            .attr('y', function(d){return yb(d.value);})
            .attr('height', function(d){return bcnf.height - yb(d.value)})
