...
(function () {
    ...
    var container_width = document.getElementById('barchart').offsetWidth,
        ...
        bcnf = {width: (container_width - 500), height: 1500, hide: 0, margin: 30, t: 60, b: 35, l: 100, r: 50},
        ccnf = {h: 40, t: 8, b: 30},
        yg, // globalyscale
        ya = d3.scaleBand().range([0, bcnf.height - (bcnf.l + bcnf.r)]), // x scale for focused chart
        xb = d3.scaleLinear().range([0, bcnf.width]),
        yc = d3.scaleBand().range([0, bcnf.height - (bcnf.l + bcnf.r)]), // x scale for context chart
        xc = d3.scaleLinear().range([ccnf.h, 0]),
        ys = d3.scaleBand().range([0, bcnf.height - (bcnf.l + bcnf.r)]),  // x scale for sorted chart
    ...
    var yAxis = d3.axisLeft(ya),
        ...
        xAxisc = d3.axisBottom(yc).tickSize(0).tickFormat(''),
        xAxiss = d3.axisBottom(ys).tickSize(2).tickValues(xc.domain().filter(function (d, i) {
            return (i % 20) == 0
    ...
    */
    ...
    bbchart.types.isSingle = {
        chart: function (data) {
            //
            // この処理は描画エリアを確保する処理だが、初めから表示しておいてよいはず
            show_chart_view();
            reset_chart();
            ...
            focus.append('g').attr('class', 'x-axis axis').attr('transform', 'translate(0,' + bcnf.height + ')').call(xAxisc);
    ...
    */
    ...
    function get_similar_gene(g) {
        return new Promise(function(resolve){
            var target_dst = refex_api + 'similarity/'+ g['id'];
            d3.queue()
