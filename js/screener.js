...
// multiple remote bloodhound
(function () {
    // screenerで取得した遺伝子リスト。各フォームの値を検証しまとめたidリスト。
    var filtered_id = [], gids = [], gene_info = [], go_suggest = [], wait = 300, default_num, your_gene_list = [];
    ...
    var bl_go = new Bloodhound({
            ...
            remote:{
                ...
                //url: 'http://localhost:8080/api/suggest?go=True&query=%QUERY',
                transform: function (response) {
    ...
    */
    ...
    //go termを選択した際の処理
    $('#go .typeahead').bind('typeahead:select', function (e, suggestion) {
        go_search(suggestion.id)
    });
    ...
    $('#refex input.tt-input').blur(function () {
    ...
    $('#refex input.tt-input').on('input', function (e) {
        setTimeout('screener()', wait)
    });

    // go term検索の入力語が空になったらgo_search()の結果（go_suggested）を初期化する
    $('#go input.tt-input').on('input', function (e) {
        var txt = $("#go input.tt-input").val();
        if (txt == ""){
            go_suggest = [];
            screener();
        }
    ...
    screener = function screener_handler(res=[]) {
    ...
    function is_input_null() {
        ...
        var refex_ttinput = $("#refex input.tt-input").val();
        var go_ttinput = $("#go input.tt-input").val();
        // search for sample specific gene の選択
        var annotations = store.specific_exp.length;

        if (refex_ttinput.length == 0 && go_ttinput.length == 0 && !annotations && your_gene_list.length == 0){
            return true
        }else{
            return false
        }
    ...
    // go idを持つ遺伝子のidを取得する
    function go_search(goid){
        var url = refex_api + "go/" + goid + "?organism=" + organism;
        fetch(url)
            .then(function (res) {
                return res.json();
            })
    ...
    function suggested_ids(){
        ...
        */
        ...
        var k = go_suggest;
        // "リストの長さが0"と"入力が無い"では、厳密には異なるのでUIとして正しい条件分岐ではないかも
        if(j.length != 0 && i.length == 0 && k.length == 0){
            // キーワード検索（j）しか入力されていない場合
            return j
        }else if(i.length != 0 && j.length == 0 && k.length == 0) {
            // your gene list(i)しか入力が無い場合
            return i
        }else if(i.length == 0 && j.length == 0 && k.length != 0){
            // go term検索の入力しか無い場合
            return k
            ...
            //iとkの積集合を計算
            var ik = intersection(i, k);
            return ik
        }else if (i.length == 0 && j.length != 0 && k.length != 0){
            var jk = intersection(j, k)
            return jk
        }else{
            // iとjの積集合を計算
    ...
    function show_gene_ids(){
        ...
        // 4. gidsしかないケース（go term検索しか入力が無いケース）では/api/infosから情報を取得する（gl==0 && gi != 0）
        // * キーワード検索語がnullになったらgene_infoを初期化しないと正しく分岐されない
        // * go term検索のinputがnullになったら"go_suggest"の値を空にする
        ...
        if (gl == 0 && gi != 0) {
            ...
            // gidとgene_infoが同じ遺伝子セットを指す場合<< 厳密では無いので要検討
