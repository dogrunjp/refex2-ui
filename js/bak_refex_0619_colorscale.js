...
(function () {
    ...
    */
    ...
    function summary_chart(c, data) {
        ...
        //name, id, symbol
        var profile = d3.select('.summary_list').append('div').attr('class', 'profile').attr('id', 'gid' + c.gid);
        ...
        function show_heatmap(d){
            get_similar_gene(d).then(function(r){
                ...
                var legend = d3.legendColor().orient('horizontal').shapeWidth(50).shapeHeight(8).shapePadding(53).labelAlign("start");
                legend.scale(co);
        ...
        function get_gene_info(i) {
            ...
            /* 静的なJSONで代用できるのでこちらのapiは廃止
            return new Promise(function(resolve){
                d3.json(`${refex_api}gene/${i}`, function(r){
                    resolve(r);
                })
            });
            */
