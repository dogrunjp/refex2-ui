...
(function(){
    ...
    var r_inf, r_infs={}, ginf, s_r_infs=[],tmp_max = 12, default_domain = [0, 12],
        chart_cols = ["value","description", "sample type"], group_cols = ["values", "description", "sample type"],
        arranged_data = [],gene_names = [], sample_data = [],
        default_legend = "sample";
    // チャートの状態
    var sorted = false, grouped = false, ppty="value";
    // チャート描画のためのパラメータ
    var cell_w = 250, cell_h = 20, cell_padding_l = 10, cell_padding_r = 60,  cell_padding_tb = 2,
    ...
    column_list_changed = function (l){
        chart_cols = l
        group_cols = chart_cols.slice(0, -1)
        group_cols.push("values")
        learn_more([gid])
    }
    ...
    function draw_axis(d) {
        ...
        if (d.key == "value" || d.key == "values"){
            ...
            d3.select(div)
                //.html(d.key);
                .html(d.key + ' <i class="fas fa-filter f-btn"></i>');
            return div
        }
    }
