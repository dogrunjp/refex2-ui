...
(function () {
    ...
    */
    ...
    var bl_gene_info = new Bloodhound({
            ...
            remote:{
                wildcard:'%QUERY',
                //url:'http://52.193.15.45/suggest?query=%QUERY',
                url: 'http://localhost:8080/suggest?query=%QUERY',
                transform: function (response) {
                    return $.map(response.results, function (bl) {
                        return {
                            value: bl
                        }
                    });
     ...
     * UIイベント定義
     * ！！利用するソースの初期化する
     * refex_gene_<organism>.jsonはデータ更新毎ファイル更新する
     * create_fene_info_file.py
     */
    ...
    */
    ...
    function parse_registered_ids() {
        ...
        if (gselected != ""){
            ...
            console.log(this)
            var tst0 = $("#refex div.tt-menu").length;
            var tst1 = $("#refex div.tt-empty").length;
            var tst = $("#refex div.tt-suggestion").length;

            console.log(tst0, tst1, tst);
            setTimeout('console.log($("#refex div.tt-suggestion").length)',5000)
